...
run_case degradation_conservative \
  "${COMMON[@]}" \
  RECOVERY_MAX_CNT=3 \
...
run_case degradation_aggressive_trigger \
  "${COMMON[@]}" \
  RECOVERY_MAX_CNT=3 \
  RECOVERY_PREV_MATCH_RADIUS=14.0 \
  RECOVERY_MIN_DIST_RATIO=0.25 \
  RECOVERY_REQUEST_MIN_TRACKS=140 \
  RECOVERY_REQUEST_LOST_RATIO=0.08 \
  RECOVERY_REQUEST_MAX_MEAN_FLOW=25 \
  RECOVERY_REQUEST_MIN_BLUR_SCORE=30 \
  RECOVERY_REQUEST_BRIGHTNESS_DELTA=20 \
  REQUEST_PREV_RADIUS=14.0 \
  REQUEST_DISTANCE_PENALTY=0.04
