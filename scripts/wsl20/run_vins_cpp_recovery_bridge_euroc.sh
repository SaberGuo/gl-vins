...
RECOVERY_REQUEST_COOLDOWN_MS="${RECOVERY_REQUEST_COOLDOWN_MS:-0}"
RECOVERY_REQUEST_MIN_SIGNAL_COUNT="${RECOVERY_REQUEST_MIN_SIGNAL_COUNT:-1}"
RECOVERY_REQUEST_REQUIRE_DEGRADATION_SIGNAL="${RECOVERY_REQUEST_REQUIRE_DEGRADATION_SIGNAL:-false}"
RECOVERY_MAX_FLOW_ERROR="${RECOVERY_MAX_FLOW_ERROR:-0}"
RECOVERY_MIN_FLOW_TRACKS="${RECOVERY_MIN_FLOW_TRACKS:-20}"
...
rosrun vins vins_node "$CONFIG" \
  ...
  _recovery_request_cooldown_ms:="$RECOVERY_REQUEST_COOLDOWN_MS" \
  _recovery_request_min_signal_count:="$RECOVERY_REQUEST_MIN_SIGNAL_COUNT" \
  _recovery_request_require_degradation_signal:="$RECOVERY_REQUEST_REQUIRE_DEGRADATION_SIGNAL" \
  _recovery_max_flow_error:="$RECOVERY_MAX_FLOW_ERROR" \
  _recovery_min_flow_tracks:="$RECOVERY_MIN_FLOW_TRACKS" \
